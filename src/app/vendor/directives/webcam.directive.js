    ...
    return {
      ...
      link: function postLink($scope, element) {
        ...
        var startWebcam = function startWebcam() {
          ...
          $scope.cameras = [];
          navigator.mediaDevices.enumerateDevices().then(gotDevices).catch(function(error){});
          ...
          function gotDevices(deviceInfos){
            ...
            for (var i = 0; i !== deviceInfos.length; ++i) {
              ...
              if(deviceInfo.kind == 'videoinput'){
                $scope.cameras.push(deviceInfo);
                console.log("cameras :::", $scope.cameras);
              }
            }
            console.log($scope.cameras[$scope.cameras.length-1].deviceId);
            
            var mediaConstraint = { 
              // video: true, 
              video: {deviceId: {exact: $scope.cameras[$scope.cameras.length-1].deviceId}},
              audio: false
            };
            alert($scope.cameras[$scope.cameras.length-1].deviceId);
